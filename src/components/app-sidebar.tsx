...
import {
  ...
  MapPin, FileText, BarChart3, ShieldCheck, Activity, Webhook, LayoutDashboard, LogOut,
} from "lucide-react";

const NAV = [
  { to: "/dashboard", label: "לוח בקרה", icon: LayoutDashboard },
  { to: "/students", label: "תלמידים", icon: Users },
  { to: "/staff", label: "כוח אדם", icon: UserCog },
  { to: "/events", label: "אירועים וטיולים", icon: Calendar },
  { to: "/graphics", label: "משימות גרפיקה", icon: Palette },
  { to: "/inspectors", label: "דיווחי בקרים", icon: ClipboardCheck },
  { to: "/vendors", label: "ספקים ואטרקציות", icon: Store },
  { to: "/communities", label: "קהילות וישיבות", icon: MapPin },
  { to: "/forms", label: "טפסים דינמיים", icon: FileText },
  { to: "/reports", label: "דוחות וייצוא", icon: BarChart3 },
] as const;

const ADMIN_NAV = [
  { to: "/admin/users", label: "ניהול משתמשים", icon: ShieldCheck },
  { to: "/admin/logs", label: "לוגים ובקרה", icon: Activity },
  { to: "/admin/webhooks", label: "Webhooks", icon: Webhook },
] as const;

export function AppSidebar() {
  const { profile, isAdmin, hasAnyRole } = useAuth();
  const loc = useLocation();
  const isActive = (to: string) => loc.pathname === to || loc.pathname.startsWith(to + "/");
  const canSeeAdmin = isAdmin || hasAnyRole(["developer", "finance"]);
      ...
      <div className="p-4 border-t border-sidebar-border space-y-3">
        <div className="flex items-center gap-3">
          ...
          <div className="overflow-hidden flex-1">
            ...
            <p className="text-[10px] text-white/40">{isAdmin ? "מנהל מערכת" : "משתמש"}</p>
          </div>
        </div>
        <button
